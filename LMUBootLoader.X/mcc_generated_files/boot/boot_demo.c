...
{    
    OUT_CONFIRM_MAINTENANCE_SetHigh();
}
...
void BOOT_DEMO_Tasks(void)
{
    ...
    // Both Green and Red LEDs blink in alternative mode to indicate BootLoader active
    if((cnt++ % 0x20000) == 0)
    {
        OUT_GreenLED_Toggle();
        if(OUT_GreenLED_GetValue() == true)
        {
            OUT_RedLED_SetHigh();
        }
        else
...
static bool EnterBootloadMode(void)
{
    ...
    // If IN_MAINTENANCE pin = 0 means that the system enter in normal operation mode if application is loaded 
    // If IN_MAINTENANCE pin = 1 means that the system enter in BootLoader mode.
    return(IN_MAINTENANCE_GetValue() != 0);
    //return false;
}
